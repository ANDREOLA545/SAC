...
class ModelCodesWidget extends HTMLElement {
  ...
  onCustomWidgetAfterUpdate(changedProperties) {
    try {
      ...
      // Show debug panel with raw keys from first row
      this._showDebug(db);
  ...
  _showDebug(db) {
    ...
    // Build debug HTML
    let html = '<div style="background:#1a2233;color:#e8f0fb;font-family:monospace;font-size:10px;padding:10px;margin-bottom:10px;border-radius:6px;overflow-x:auto;">';
    html += '<b style="color:#fbbf24">DEBUG — Keys del primer row:</b><br><br>';
    keys.forEach(k => {
      const v = JSON.stringify(row0[k]);
      html += `<span style="color:#6baee8">${k}</span>: <span style="color:#5dbf7a">${v}</span><br>`;
    });
    html += '</div>';

    // Inject into shadow root before the main content
