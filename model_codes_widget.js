...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Tabla ── */
  .table-wrapper { overflow-x: auto; background: #fff; width: 100%; flex: 1; }
...
class ModelCodesWidget extends HTMLElement {
  ...
  // Handles: "09.2025", "202509", "2025-09", "October 2025", "Oct 2025"
  _parseYYYYMM(raw) {
    if (!raw) return null;
    ...
    const textMatch = s.match(/^([A-Za-z]+)\s+(\d{4})$/);
    if (textMatch) {
      const m = MONTHS[textMatch[1].toLowerCase()];
      if (m) return parseInt(textMatch[2]) * 100 + m;
    }
    // "2025 October" or "2025 Oct"
    const textMatch2 = s.match(/^(\d{4})\s+([A-Za-z]+)$/);
    if (textMatch2) {
      const m = MONTHS[textMatch2[2].toLowerCase()];
      if (m) return parseInt(textMatch2[1]) * 100 + m;
    }
    // MM.YYYY  e.g. "09.2025"
  ...
  _renderTable() {
    ...
    this._rowGroups.forEach(group => {
      ...
      // Frozen End en rojo solo si Frozen End < Fecha Entrega (problema real)
      const frozenDate  = this._parseDate(group.frozen);
      const fechaDate   = this._parseDate(group.fecha);
      const frozenAlert = frozenDate && fechaDate && frozenDate < fechaDate;

      [
        { val: group.material, cls: 'material' },
        { val: group.fecha,    cls: 'date'     },
        { val: group.docDate,  cls: 'date'     },
        { val: group.frozen,   cls: 'frozen' + (frozenAlert ? ' frozen-active' : '') },
      ].forEach(({ val, cls }) => {
      ...
      if (hasPast) {
        ...
        if (!pastCell || pastCell.qty === null) {
          ...
          innerP.textContent = this._fmtQty(pastCell.qty);
  ...
  // ── Helpers ───────────────────────────────────────────────────
  // Format quantity → locale string with thousands separator
  _fmtQty(qty) {
