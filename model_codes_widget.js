...
class ModelCodesWidget extends HTMLElement {
  ...
  _processRows(rows) {
    ...
    // CalMonth format: YYYYMM (e.g. 202510)
    const todayNum = this._todayYYYYMM();

    // Collect all unique periods from data
    const allPeriods = [...new Set(rows.map(r => r.yearMonth))]
      .filter(p => p && p.length === 6)
      .map(p => parseInt(p))
      .filter(p => !isNaN(p))
      .sort((a, b) => a - b);

    // Split into PAST (< current month) and FUTURE (>= current month, up to 12)
    const pastPeriods  = allPeriods.filter(p => p < todayNum);
    const futurePeriods = allPeriods
      .filter(p => p >= todayNum)
      .slice(0, 13); // current month + 12 more

    this._pastPeriods   = pastPeriods.map(String);
    this._futurePeriods = futurePeriods.map(String);
    // _periods = all visible columns (used for cell lookup)
  ...
  // Format YYYYMM → "Oct 2025"
  _formatPeriod(yyyymm) {
    const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    const s = String(yyyymm);
    if (s.length !== 6) return s;
    const y = s.slice(0, 4);
    const m = parseInt(s.slice(4)) - 1;
    return `${months[m]} ${y}`;
  }
