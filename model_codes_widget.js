...
class ModelCodesWidget extends HTMLElement {
  ...
  _processRows(rows) {
    ...
    // Collect all unique raw period values from data
    const rawPeriods = [...new Set(rows.map(r => r.yearMonth).filter(Boolean))];

    // Normalize each raw value → { raw, num } where num = YYYYMM integer
    const normalized = rawPeriods.map(raw => ({ raw, num: this._parseYYYYMM(raw) }))
      .filter(p => p.num !== null)
      .sort((a, b) => a.num - b.num);

    const pastNorm   = normalized.filter(p => p.num < todayNum);
    const futureNorm = normalized.filter(p => p.num >= todayNum).slice(0, 13);

    this._pastPeriods   = pastNorm.map(p => p.raw);
    this._futurePeriods = futureNorm.map(p => p.raw);
  ...
  // Format raw CalMonth value → readable label e.g. "Sep 2025"
  _formatPeriod(raw) {
