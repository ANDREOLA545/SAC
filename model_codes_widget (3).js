...
class ModelCodesWidget extends HTMLElement {
  ...
  onCustomWidgetAfterUpdate(changedProperties) {
    try {
      // SAC passes dataBinding via the widget's property, not as argument
      // Try all known SAC patterns
      const db = changedProperties?.dataBinding
              ?? changedProperties?.myDataBinding
              ?? this.dataBinding
              ?? this.myDataBinding
              ?? null;
      ...
      const debugInfo = {
        changedPropertiesKeys: changedProperties ? Object.keys(changedProperties) : [],
        hasDataBinding: !!this.dataBinding,
        hasMyDataBinding: !!this.myDataBinding,
        db_type: db ? typeof db : 'null',
        db_keys: db ? Object.keys(db) : [],
        myDB_keys: db?.myDataBinding ? Object.keys(db.myDataBinding) : [],
        data_length: db?.myDataBinding?.data?.length ?? db?.data?.length ?? 'n/a',
        row0_keys: db?.myDataBinding?.data?.[0] ? Object.keys(db.myDataBinding.data[0])
                 : db?.data?.[0] ? Object.keys(db.data[0]) : [],
        row0_sample: db?.myDataBinding?.data?.[0] ?? db?.data?.[0] ?? null,
      };
      this._showDebugInfo(debugInfo);
  ...
  _showDebugInfo(info) {
    let html = '<div style="background:#1a2233;color:#e8f0fb;font-family:monospace;font-size:10px;padding:10px;margin-bottom:10px;border-radius:6px;overflow-x:auto;white-space:pre-wrap;">';
    html += '<b style="color:#fbbf24">⚙ DEBUG dataBinding</b>\n\n';
    html += JSON.stringify(info, null, 2);
    html += '</div>';
    let dbg = this._shadow.getElementById('debugPanel');
    if (!dbg) {
      dbg = document.createElement('div');
      dbg.id = 'debugPanel';
      const root = this._shadow.querySelector('.root');
      root.insertBefore(dbg, root.firstChild);
    }
    dbg.innerHTML = html;
  }
  ...
  _extractRows(db) {
    ...
    console.log('[ModelCodes] row[0] keys:', Object.keys(data[0]));
    console.log('[ModelCodes] row[0]:', JSON.stringify(data[0]));
