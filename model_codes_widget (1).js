...
class ModelCodesWidget extends HTMLElement {
  ...
  _extractRows(db) {
    ...
    // Log first row so we can see the exact structure SAC sends
    ...
    return data.map(row => {
      ...
      return {
        ...
        codTexto:  this._normalizeCode(feedVal(row, 'feed_cod_texto')),
        codFondo:  this._normalizeCode(feedVal(row, 'feed_cod_fondo')),
      };
    });
  }

  // Normalize SAC dimension value → clean digit string ("1","2","3","4")
  // SAC may send: { id:"0001", label:"1" }, or "1", or 1, or " 1 "
  _normalizeCode(item) {
    ...
    for (const c of candidates) {
      ...
      // Strip whitespace and leading zeros, keep the significant digit
